...
subtest 'run_ok timeout' => sub {

  my $run;

  Test::Alien::run_timeout(2);

  is(
    intercept { $run = run_ok [$^X, 'corpus/test_alien/run-forever.pl'], 'foobar' },
    array {
      event Ok => sub {
        call pass => F();
        call name => 'foobar';
      ...
      event Diag => sub {};
      event Diag => sub {};
      event Diag => sub { call message => match qr/exception: timeout\./ };
      end;
    },
  );
