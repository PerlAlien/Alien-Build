...
Here is the L<alienfile> that you might have:
 ...
 gather [
   [ 'pkg-config --modversion libfoo', \'%{alien.runtime.version}' ],
   [ 'pkg-config --cflags     libfoo', \'%{alien.runtime.cflags}'  ],
   [ 'pkg-config --libs       libfoo', \'%{alien.runtime.libs}'    ],
 ];
...
variable C<%{alien.install.download> as a placeholder for the archive 
...
C<%{alien.install.prefix}> is the location where the library should be 
...
which can sometimes be useful:
 ...
 % af install --dry-run --before build bash
 Alien::Build::Plugin::Core::Download> decoding html
 Alien::Build::Plugin::Core::Download> candidate *https://www.libfoo.org/download/libfoo-1.2.4.tar.gz
 Alien::Build::Plugin::Core::Download> candidate  https://www.libfoo.org/download/libfoo-1.2.3.tar.gz
 Alien::Build::Plugin::Core::Download> candidate  https://www.libfoo.org/download/libfoo-1.2.2.tar.gz
...
L<alienfile> into the runtime properties:
 ...
 gather [
   # standard:
   [ 'foo-config --version libfoo', \'%{alien.runtime.version}' ],
   [ 'foo-config --cflags  libfoo', \'%{alien.runtime.cflags}'  ],
   [ 'foo-config --libs    libfoo', \'%{alien.runtime.libs}'    ],
   # non-standard
   [ 'foo-config --bar-baz libfoo', \'%{alien.runtime.bar_baz}' ],
 ];
