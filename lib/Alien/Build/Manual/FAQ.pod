...
can turn network fetch back on by unsetting the environment variable C<NO_NETWORK_TESTING> or
by setting C<ALIEN_INSTALL_NETWORK> to true, or by unsetting both of them.  These environment variables
are designed for environments that don't ever want to install aliens that require downloading
