...
to provide the normal C<pkg-config> fileds then you could do something like this:
 ...
 share {
   ...
   build [
     '%{make} PREFIX=%{alien.runtime.prefix}',
     '%{amek} install PREFIX=%{alien.runtime.prefix}',
   ];
 };
 
 gather [
   [ 'foo-config', '--version', \'%{alien.runtime.version}' ],
   [ 'foo-config', '--cflags',  \'%{alien.runtime.cflags}'  ],
   [ 'foo-config', '--libs',    \'%{alien.runtime.libs}'    ],
 ];
...
If you have a patch file you can use diff:
 ...
 share {
   ...
   patch [ '%{patch} -p1 < %{alien.install.patch}/mypatch.diff' ];
   build [ ... ] ;
 }
...
MSYS plugin and use a command sequence to do the build like this:
 ...
 share {
   ...
   build [
     ...
     'sh configure --prefix=%{alien.install.prefix} --disable-shared',
     '%{make}',
     '%{make} install',
   ];
 };
